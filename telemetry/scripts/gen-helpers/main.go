...
func main() {
	...
	flag.StringVar(&outPath, "out", "cli/internal/telemetry/events.gen.go", "events.gen.go output path (tx.gen.go and emit.gen.go land in the same directory)")
	flag.Parse()
	...
	// Three sibling files share the same package + ordering
	// guarantees. events.gen.go stays runtime-free (wire types +
	// enums only); tx.gen.go and emit.gen.go carry the helper
	// shells that bind to the hand-written runtime in the same
	// package (Client, fireCommandInvoked, stampInvocation*).
	writeFormatted(outPath, func(w io.Writer) error { return emitFile(w, root) })
	writeFormatted(filepath.Join(outDir, "tx.gen.go"), emitTxFile)
	writeFormatted(filepath.Join(outDir, "emit.gen.go"), emitEmitFile)
}

// writeFormatted runs the supplied emitter into a buffer, gofmts
// it, and writes to path. On gofmt failure the unformatted output
// is saved next to the target so the structural diff is visible.
func writeFormatted(path string, emit func(io.Writer) error) {
	var buf bytes.Buffer
	if err := emit(&buf); err != nil {
		log.Fatalf("emit %s: %v", filepath.Base(path), err)
	}
	formatted, err := format.Source(buf.Bytes())
	if err != nil {
		_ = os.WriteFile(path+".unformatted", buf.Bytes(), 0o644)
		log.Fatalf("gofmt %s (raw output at %s.unformatted): %v", filepath.Base(path), path, err)
	}
...
func emitDoc(w io.Writer, doc string) {
	...
	for _, line := range strings.Split(doc, "\n") {
		...
		if line == "" {
			// Preserve paragraph breaks. A blank line in the
			// source becomes a bare `//` separator so godoc
			// renders the comment as multiple paragraphs.
			fmt.Fprintln(w, "//")
			continue
		}
		fmt.Fprintf(w, "// %s\n", line)
...
// ---------------------------------------------------------------------------
// tx.gen.go + emit.gen.go file emitters
// ---------------------------------------------------------------------------

const txFileHeader = `// Code generated by telemetry/scripts/gen-helpers. DO NOT EDIT.
//
...
const emitFileHeader = `// Code generated by telemetry/scripts/gen-helpers. DO NOT EDIT.
//
...
// emitTxFile generates tx.gen.go: BeginX constructors and (*XTx).End
// methods for every long-running command. Body shape matches the
// hand-written version retired alongside this commit; see doc.go for
// the contract.
func emitTxFile(w io.Writer) error {
	mustWrite(w, txFileHeader)
	for _, cmd := range commands {
		if !cmd.LongRun {
			continue
		}
...
// articleFor picks "a" vs "an" for the leading article in generated
// doc strings. Command names that start with a vowel sound take "an"
// (MCP, LSP read as "em-see-pee", "el-ess-pee"). Drives doc cosmetic
// only - the runtime doesn't care.
func articleFor(goName string) string {
	if goName == "" {
		return "a"
	}
	switch goName[0] {
	case 'A', 'E', 'I', 'O', 'U', 'L', 'M', 'N', 'R', 'S', 'F', 'H', 'X':
		// L, M, N, R, S, F, H, X all start with a vowel sound when
		// spelled out as an initialism (el, em, en, ar, es, eff, aitch, ex).
		return "an"
	}
	return "a"
}
...
func emitEnd(w io.Writer, cmd commandSpec) {
	emitDoc(w, fmt.Sprintf("End finalises the %s transaction: recovers any in-flight panic,\nstamps the emit-side base fields on tx.props, defaults Outcome\nfrom (recovered panic | ctx.Err() | success), enqueues the\nenvelope, and re-panics if the command body panicked.\n\nMUST be called as `defer tx.End(ctx)` directly - see doc.go for\nthe load-bearing recover-frame requirement.", cmd.CmdName))
	fmt.Fprintf(w, "func (tx *%sTx) End(ctx context.Context) {\n", cmd.GoName)
	fmt.Fprint(w, "\tr := recover()\n")
...
func emitEmitFile(w io.Writer) error {
	...
	for _, cmd := range commands {
		...
		emitOneShotHelper(w, cmd)
	}
	return nil
}

func emitOneShotHelper(w io.Writer, cmd commandSpec) {
