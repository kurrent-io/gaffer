...
Each `[env.X]` block in `gaffer.toml` gains a **Deploy** lens leading the block, ahead of its status roll-up. It opens the deploy plan for the whole project against that env in an editor-area webview, to review and then deploy.
