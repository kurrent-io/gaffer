...
Deploying from the editor: each `[env.X]` block in `gaffer.toml` gains a **Deploy** lens leading the block, ahead of its status roll-up, and the per-projection **Manage...** menu gains a **Deploy** action. Both open the deploy plan in an editor-area webview, to review and then deploy - the lens scoped to the whole project against that env, the menu action scoped to just that projection (resolving its own change, and bundling a recreate when one's needed).
