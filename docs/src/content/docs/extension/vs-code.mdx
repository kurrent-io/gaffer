---
title: VS Code
description: Install the KurrentDB Gaffer VS Code extension for inline run/debug, breakpoints, state inspection, deploy status and plans, and projection-API autocomplete.
---
...
<ThemeImage
	base="/vscode/vscode-debug-paused"
	alt="Paused at a breakpoint inside a handler: the call stack and breakpoints in the debug side bar, and the Gaffer panel showing the current event and per-stream state"
/>
...
<ThemeImage
	...
	alt="A deploy plan in an editor tab: updates flagged as logic changes with Diff actions, unchanged projections, and invalid projections with their compile errors inline"
/>
...
<ThemeImage
	base="/vscode/vscode-toml-lenses"
	alt="gaffer.toml with Debug, Debug from, and Manage CodeLenses on each projection, per-projection status dots, and the environment's deploy status summary"
/>
...
<ThemeImage
	...
	alt="A projection's deploy timeline: deploy, disable, enable, and rollback versions on a graph, with the selected rollback's metadata and actions in a detail panel"
/>
...
<ThemeImage
	base="/vscode/vscode-run-status"
	alt="The Gaffer panel after a fixture run: the Status view's run summary and the State view's per-stream state slices"
/>
