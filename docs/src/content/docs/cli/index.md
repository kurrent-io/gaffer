...
- **`--connection`**: ad-hoc connection string for a single invocation of `gaffer dev`, `gaffer diff`, `gaffer status`, `gaffer deploy`, `gaffer start`, `gaffer stop`, or `gaffer delete`. Overrides `--env` and the configured environment.
