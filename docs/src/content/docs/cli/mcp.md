...
- **`deploy_apply`**: deploy from `gaffer.toml`, mirroring `gaffer deploy` - the same compile-and-diagnostics preflight (no bypass), the same per-item results as `gaffer deploy --json`, and every write ledger-stamped. A production deploy asks you to confirm through your MCP client, with the changed projections and cautions named in the prompt. A plan containing `resetOnLogicChange` rebuilds destroys state, so it always asks; on production that confirmation requires typing the environment name.
