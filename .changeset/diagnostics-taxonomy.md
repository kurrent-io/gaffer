...
- **Throwing quirks now also raise a diagnostic.** A quirk that throws (e.g. `quirk.event.bodyCast`, `quirk.serialize.nonFinite`) exposes a `diagnostics` array on the thrown error (Go error types and the JS `ProjectionError`, propagated through the testing library), carrying the quirk that threw plus any that fired earlier in the same event - not just a single `compatCode`. Errors are also enriched with `compatDescription` and `compatFixedIn`.
