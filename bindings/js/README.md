...
- **`ProjectionSession`** - the session class. Construct with source + options, then `feed()`, `getState()` / `getSharedState()` / `getResult()` (with `getStateJson()` / `getSharedStateJson()` / `getResultJson()` parsed-JSON variants), `setState()`, `getSources()`, `getPartitionKey()`, `onEmit()` / `onLog()` / `onStateChanged()`, `dispose()`. The fallible methods throw a `ProjectionError` subclass on a runtime error. The state getters return `null` only for a genuinely absent partition or value, never to mask an error. An exception thrown inside an `onEmit` / `onLog` / `onStateChanged` callback is re-thrown from the `feed()` / `getResult()` / `getPartitionKey()` call that triggered it rather than being swallowed. Call `dispose()` (or use a `using` declaration) to release the session promptly; a `FinalizationRegistry` is a fallback that frees a session leaked without `dispose()`, but it should not be relied on.
- **Error classes** - `ProjectionError` base plus `InvalidProjectionError`, `CompilationTimeoutError`, `InvalidArgumentError`, `ProjectionHandlerError`, `ExecutionTimeoutError`, `MalformedEventError`, `StateSerializationError`, `ProjectionTransformError`. All carry structured fields: `code`, `description`, event context where applicable, and the quirk-compat trio (`compatCode` / `compatDescription` / `compatFixedIn`) with `diagnostics` when an upstream-quirk-compat path threw.
- **Types** - `ProjectionEvent`, `EmittedEvent`, `FeedResult`, `ProjectionInfo`, `SessionOptions`, `EventContext`, `Diagnostic`, `SourceRange`, `SourcePosition`, `DiagnosticSeverity`.

The TypeScript declarations are the source of truth for the surface; see [`src/index.ts`](https://github.com/kurrent-io/gaffer/blob/main/bindings/js/src/index.ts) for the full export list.
