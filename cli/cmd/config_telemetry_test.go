...
func TestConfigTelemetryOn_FreshMintsAndNotifies(t *testing.T) {
	...
	// Notice on stderr.
	if !strings.Contains(stderr, "gaffer.kurrent.io/telemetry") {
...
func TestConfigTelemetryOn_OnThenOnNoDoubleNotice(t *testing.T) {
	...
	if !strings.Contains(stderr1, "gaffer.kurrent.io/telemetry") {
	...
	if strings.Contains(stderr2, "gaffer.kurrent.io/telemetry") {
