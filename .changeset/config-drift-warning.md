...
`gaffer status --json` now emits an object: a `projections` array (the previous per-projection entries) plus a `configDrift` array of `{"knob", "server", "local"}` when the target diverges, so machine consumers see the divergence without a second call. `gaffer deploy --json` is unchanged; its warning prints on stderr, keeping the stdout payload clean while CI logs still show it.
