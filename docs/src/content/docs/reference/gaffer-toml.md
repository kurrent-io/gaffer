...
- **`default`**: optional bool. Exactly one environment may set `default = true`. With a default, `--env` is optional and the default is used; with no default, `--env` is required; two defaults is a config error.
