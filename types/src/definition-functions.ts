import type { FromAllChain, FromCategoryChain, FromStreamChain } from "./chains.ts";
...
export interface FromCategoryFn {
	...
	<S extends State = State>(categories: [string, ...string[]]): FromCategoryChain<S>;
	<S extends State = State>(...categories: [string, ...string[]]): FromCategoryChain<S>;
}
...
export interface FromStreamsFn {
	 ...
	 *   });
	 */
	<S extends State = State>(streamIds: [string, ...string[]]): FromStreamChain<S>;
	<S extends State = State>(...streamIds: [string, ...string[]]): FromStreamChain<S>;
}
...
export interface OnEventFn {
	 ...
	 * }));
	 */
	<S extends State = State>(eventName: string, eventHandler: EventHandler<S>): void;
}
...
export interface OnAnyFn {
	 ...
	 * }));
	 */
	<S extends State = State>(eventHandler: EventHandler<S>): void;
}
