...
- `gaffer dev` gained `--env <name>` to select an environment. With a default set, `--env` is optional; with none, it is required. `--connection` is an ad-hoc override that beats both `--env` and the configured environment. The MCP `list_events` and live `run` tools take the same `env` argument.
