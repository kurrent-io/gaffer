...
```sh
gaffer dev order-count --events fixtures/orders.json --debug --debug-port 4711
```
...
Gaffer's debug server is a standard DAP-over-TCP endpoint. Attach your editor's debugger to the port gaffer is listening on.
...
1. Run `gaffer dev <projection> --events <fixture> --debug --debug-port 4711` in a terminal.
