...
- The env-block **Deploy** lens is hidden unless the CLI can run the deploy spawns, and the per-projection **Manage...** menu drops any row its CLI can't serve - **Deploy** and **History** against the CLI's own command listing, **Diff against deployed** and the operate verbs against the methods the language server advertises. An environment left with no available rows collapses to a single **Unsupported** notice, matching the existing **Unavailable** and **Sign in** collapses, and the **Manage...** lens itself is hidden when nothing at all is available.
