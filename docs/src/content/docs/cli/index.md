...
| [`gaffer deploy [projection]`](./commands.md#gaffer-deploy) | Create or update projections on an environment: create the new ones, update the changed ones, skip the in-sync ones. Compiles every projection locally first and refuses the run if any has errors (`--force` bypasses). Shows the plan and confirms before applying (`--yes` skips); a production server is extra-guarded and refuses `--force`. |
