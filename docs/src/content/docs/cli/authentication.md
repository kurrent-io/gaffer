...
- **For local development**, leave `KURRENTDB_OAUTH_CLIENT_SECRET` unset and run [`gaffer auth --env <name>`](./commands.md#gaffer-auth) once. It signs in through the browser and stores a token that refreshes automatically. The [VS Code extension](../extension/vs-code.mdx#authentication) offers the same sign-in inline.
