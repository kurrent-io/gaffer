...
- **`user_cert_file`** / **`user_key_file`**: optional paths to an X.509 user certificate and its private key, for authenticating to KurrentDB with a client certificate. Both must be set together. The certificate is presented in the TLS handshake, so the connection must use TLS. This is independent of OAuth - a connection may use a client certificate and bearer tokens together. Enterprise / self-hosted only, not available on Kurrent Cloud.
...
Like `connection`, the cert paths expand `${VAR}` references and resolve relative to the project root when not absolute, so a relative path works regardless of the directory `gaffer` runs from.
