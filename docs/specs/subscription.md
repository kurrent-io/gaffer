...
The runtime handles `$includeLinks` filtering internally. When `$includeLinks` is false (default), the runtime drops raw link events (`$>` event type) and resolved-from-link events (events with `linkMetadata` set) in `Feed()`. Consumers should always subscribe with `resolveLinks: true` and pass everything through - the runtime decides what to keep.
