...
WorkspaceSymbolProvider. Parses `gaffer.toml` and projection JS, emits
lenses with embedded command descriptors. The VS Code extension consumes
this as its source of truth for parsing and lensing rather than
re-implementing in-process; other editors connect over stdio the same way.
...
- Schemas in `telemetry/schemas/` (CUE); generated TS / Go / JSON Schema
  under `telemetry/generated/` (gitignored, produced by `just telemetry build`).
