...
- `--stream` requires `--json`; on its own it is refused, since a plain or interactive run already streams its own progress.
- It affects the apply only. `--dry-run` still prints the one-shot plan envelope, and a run with nothing to deploy emits a single zeroed `deploy_summary` so a streaming consumer always ends on one.
